...
def add_parcellations(hdf5file, parcfile, output, specific_tags=None):
    # If specific_tags is None, include all parcellations found
    import dolfin as df

    # Read the mesh and mesh data from .h5:
    mesh = df.Mesh()
    hdf = df.HDF5File(mesh.mpi_comm(), hdf5file, "r")
    ...
    hdf = df.HDF5File(mesh.mpi_comm(), output, "w")
    hdf.write(mesh, "/mesh")            
    ...
    xdmfname = ".".join(Z.output.split(".")[0:-1]) + "-tmp.xdmf"
    xdmf = df.XDMFFile(mesh.mpi_comm(), xdmfname)
...
if __name__ =='__main__':
    ...
    parser.add_argument("-pf", "--parcfile", type=str)     
    parser.add_argument("-o", "--output", type=str) 
    parser.add_argument("-a", "--add", type=int, nargs='+') 

    Z = parser.parse_args() 
    add_parcellations(Z.h5file, Z.parcfile, Z.output, Z.add)
